...
PasswordForm::PasswordForm(PassWordClassify pwc, QWidget *parent) : QWidget(parent)
{
	m_pwc = pwc;
	__setupUI();
}
...
void PasswordForm::__setupUI()
{
	...
	switch (m_pwc)
	{
	case PassWordClassify::EXITMAINPROCESS:
		this->setWindowTitle(QStringLiteral("退出密码"));
		break;
	case PassWordClassify::ENTRYCAMERASET:
		this->setWindowTitle(QStringLiteral("相机设置密码"));
		break;
	}
...
void PasswordForm::__confirmPassword()
{
	...
	if (m_pPassEdit->text() == m_strPassWord)
	{
...
void PasswordForm::closeEvent(QCloseEvent *event)
{
	m_pPassEdit->setText("");
}